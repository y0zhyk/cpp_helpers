...
MemoryStream::MemoryStream(const Byte* data, const size_t size)
{
    ...
    if (data == nullptr)
        throw std::invalid_argument(
            "MemoryStream::MemoryStream(): data == nullptr");

    data_.assign(data, data + size);
}

size_t
MemoryStream::size() const noexcept
{
    return data_.size();
}
...
const Bytes&
MemoryStream::bytes() const noexcept
{
    return data_;
}
...
std::string
MemoryStream::ToString() const noexcept
{
    std::stringstream ss;
...
MemoryStream&
operator<<(MemoryStream& ms, const Bytes& bytes)
{
    ms << bytes.size();
...
MemoryStream&
operator>>(MemoryStream& ms, Bytes& bytes)
{
    size_t size = 0;
    ms >> size;
    bytes.resize(size);
