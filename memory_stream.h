...
public:
  ...
  // Initializes a new instance of the MemoryStream class based on the specified
  // block of bytes.
...
MemoryStream &operator<<(MemoryStream &ms, const std::basic_string<T> &str) {
  ms << str.length();
  ms.Write(reinterpret_cast<const Byte *>(str.data()),
           sizeof(T) * str.length());
  return ms;
}
...
MemoryStream &operator>>(MemoryStream &ms, std::basic_string<T> &str) {
  ...
  ms >> length;
  str.resize(length);
  ms.Read(reinterpret_cast<Byte *>(const_cast<T *>(str.data())),
          sizeof(T) * str.length());
  return ms;
}
