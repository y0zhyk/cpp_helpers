...
class Lazy {
 public:
  ...
  const Result& operator()() const { return const_cast<Lazy&>(*this)(); }

  Result& operator()() {
    if (!value_) value_ = function_();
    return *value_;
  }
...
template <class Function>
Lazy<typename std::remove_reference<Function>::type> MakeLazy(
    Function&& function) {
  return Lazy<typename std::remove_reference<Function>::type>(
