...
// The ISymmetricCipher interface represents the functionality of a cryptographic cipher used for encryption and decryption.
class ISymmetricCipher {
 ...
 public:
    ...
    virtual bool Encrypt(const Bytes& plain_text, Bytes& cipher_text) const = 0;
    virtual bool Encrypt(const Byte* plain_text, const size_t plain_text_size, Bytes* cipher_text, size_t& cipher_text_size) const = 0;
    // Decrypts data with the key and the parameters set
    virtual bool Decrypt(const Bytes& cipher_text, Bytes& plain_text) const = 0;
    virtual bool Decrypt(const Byte* cipher_text, const size_t cipher_text_size, Bytes* plain_text, size_t& plain_text_size) const = 0;
    // Sets a secret key to use in the symmetric cipher.
    virtual void SetKey(const IAESKey* key) = 0;
...
// The ISymmetricKey interface represents the functionality of a cryptographic key used for encryption and decryption.
class ISymmetricKey {
...
class IHasher {
    ...
    virtual bool Update(const Bytes& message) = 0;
    ...
    virtual bool GetHash(Byte* hash_text, const size_t cipher_text_size, Bytes* plain_text, size_t& hash_size) const = 0;
    // Resets.
    virtual void Reset() = 0;
    // Compure hash.
    virtual bool ComputeHash(const Bytes& message, Bytes& hash) const = 0;
    virtual bool ComputeHash(const Byte* message, const size_t message_size, Bytes* hash, size_t& hash_size) const = 0;
...
class ISecurityFactory {
	...
	virtual ISymmetricKey* CreateSymmetricKey(const Bytes& key) const = 0;
	virtual ISymmetricKey* CreateSymmetricKey(const Byte* key_data, const size_t size) const = 0;
    // Creates a symmetric cipher with given parametrs
    virtual ISymmetricCipher* CreateSymmetricCipher(const ISymmetricKey* key = nullptr, const ISymmetricCipher::Mode mode = ISymmetricCipher::ECB, const const Bytes& iv) const = 0;
    // Creates hasher with given parame;
    virtual IHasher* CreateHasher(const IHasher::HashAlgorithm algorithm) = 0;
